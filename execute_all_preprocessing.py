...
print("\n==== BPM detector ====================")
pid_get_bpm = subprocess.Popen(["python", "bpm_detector.py", "--filename", "./" + args.input + ".wav"], stdout=subprocess.PIPE)
terminal_out_bpm, err1 = pid_get_bpm.communicate()
print("bpm detector process:", pid_get_bpm, " || Any error? ", err1)
terminal_out_bpm = np.round(float(terminal_out_bpm.split()[-1])) #least integer
print(terminal_out_bpm)

print("\n==== Noise Reduction =================")
noise_reduction_output_path = "./" + filename + "_reduction"
pid_noise_reduction = subprocess.Popen(["python", "noise_reduction.py", "--input", "./" + args.input + ".wav", "--output", noise_reduction_output_path + ".wav"], stdout=subprocess.PIPE)
