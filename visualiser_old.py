...
def print_words(real_in, reference, hypothesis):
    ...
    print("REFERENCE:", " ".join(map(str, answers)),
          "REFERENCE_WORD:", real_in[answers[0]] if answers else "")
    print("HYPOTHESIS_INDEX:", hypothesis,
          "HYPOTHESIS_WORD:", real_in[hypothesis])
        ...
        with tf.Graph().as_default():
            ...
            with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto()) as sess:
                ...
                for i in range(n_test_inputs // BATCH_SIZE):
                    ...
                    pred_flat = (tester.get_result(
                        sess, batch_xs, batch_ys)).flatten()
...
if __name__ == '__main__':
    ...
    #         run_test_musicnet(inference_file_path=args.inference_file_path,
    #                           filename=args.inference_file_name, save_results=True)
    # else:
    run_test()
