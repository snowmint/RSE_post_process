            ...
            for xx in range(-1, 2):
                if (y + yy >= 0 and y + yy < height):
                    if (x + xx >= 0 and x + xx < width):
                        binary_x += post_processing[y + yy][x + xx] * sobel1[index]
                        binary_y += post_processing[y + yy][x + xx] * sobel2[index]
                index += 1
...
post_processing_diff = interpolation_array(post_processing, height, width, 5) #interpolation_rate
...
overlap = original_prediction * ground_truth#ground_truth_front300 # Logical AND
...
# plt.show()
vls = np.load("./" + args.input + ".npy")
track = pypianoroll.BinaryTrack(pianoroll=post_processing_diff.T) #post_processing_diff #pianoroll=vls[0].T>0.9
multi_track = pypianoroll.Multitrack(tracks=[track])
