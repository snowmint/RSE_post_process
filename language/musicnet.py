...
# "/piano_ver_Pneumatic.npy", musicnet_test
MUSICNET_TEST = "/musicnet_validation.npy"
print(MUSICNET_TEST)
...
class Musicnet(LanguageTask):
    def __init__(self) -> None:
        # how long is one input sequence (e.g. 2048)
        self.window_size = cnf.musicnet_window_size
        # how many inputs there are in mmap partial load
        self.mmap_count = cnf.musicnet_mmap_count
        self.data_dir = str("./")  # str(Path(cnf.musicnet_data_dir))
        self.training_set = []
        self.validation_set = []
        self.testing_set = []
    ...
    def crop(self, xy_set):
        ...
        inputs = np.expand_dims(
            xy_set[:, 0, midpoint - half_window:midpoint + half_window], axis=1)
        labels = np.expand_dims(
            xy_set[:, 1, midpoint - half_window:midpoint + half_window], axis=1)  # crops padding
    ...
    def load_training_dataset_mmap(self):
        ...
        for i in range(10):
            ...
            self.training_set += copy.deepcopy(
                self.crop(loaded[i * part_size:(i + 1) * part_size]))
